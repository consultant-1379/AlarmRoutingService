 ...
 */
@Named("eventTypeHandler")
public class EventTypeHandler extends AbstractEventHandler implements EventInputHandler, Controllable {
    ...
    public void onEvent(final Object flowInputEvent) {
        ...
        final List<AlarmRoute> eventTypeMatchedAlarmRoutes = new ArrayList<AlarmRoute>();
        try {
            for (final AlarmRoute alarmRoute : alarmRoutes) {
                if (null == processedAlarmEvent.getEventType() || null == alarmRoute.getEventType() || processedAlarmEvent.getEventType().isEmpty()
                        || alarmRoute.getEventType().isEmpty()) {
                    eventTypeMatchedAlarmRoutes.add(alarmRoute);
                } else {
                    if (MetaDataProcessor.isAttributePresent(processedAlarmEvent.getEventType(), alarmRoute.getEventType())) {
                        eventTypeMatchedAlarmRoutes.add(alarmRoute);
                    }
                }
            }
            if (!eventTypeMatchedAlarmRoutes.isEmpty()) {
                alarmRouteHandlerEvent.setAlarmRoutes(eventTypeMatchedAlarmRoutes);
                log.debug("The incoming alarm matched with the EventType filtered alarmRoutes count :{} ", eventTypeMatchedAlarmRoutes.size());
                sendToAllSubscribers(alarmRouteHandlerEvent);
            } else {
                log.debug("The incoming alarm not matched with any EventType filtered alarmRoute.");
            }
